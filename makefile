...
SRCS = $(filter-out $(SRCS_BINARY), $(patsubst $(PATH_SRC)/%,%,$(call rwildcard,$(PATH_SRC),*.c)))$(filter-out $(SRCS_BINARY), $(patsubst $(PATH_SRC)/%,%,$(call rwildcard,$(PATH_SRC),*.cpp)))
...
PATH_ALL += $(UNIX_PATH_BUILD) $(UNIX_PATH_BUILD_BINARY) $(UNIX_PATH_BUILD_OBJS) $(UNIX_PATH_BUILD_BINARIES)
# END MACRO-UNIX
